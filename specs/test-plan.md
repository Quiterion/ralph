...
- `cmd_list` parses `.wiggum/panes.json` to display active panes
...
- Writes to `.wiggum/current_prompt.md`
...
| 3.7 | Template in defaults | User hasn't customized | Uses `$WIGGUM_DEFAULTS/prompts/` |
| 3.8 | Template in project | User has `.wiggum/prompts/foo.md` | Uses project version |
| 3.9 | Feedback in ticket | Ticket has `## Feedback` section | Extracted into `{FEEDBACK}` |
| 3.10 | Prompt file created | After spawn | `.wiggum/current_prompt.md` exists in worktree |
...
- Only checks Claude config when `WIGGUM_AGENT_CMD` is `claude`
...
|----|-------------|-------|----------|
| 4.1 | Claude with vim mode | `~/.claude.json` has `"editorMode": "vim"` | `WIGGUM_EDITOR_MODE=vim` |
| 4.2 | Claude with normal mode | `"editorMode": "normal"` | `WIGGUM_EDITOR_MODE=normal` |
| 4.3 | Claude no editorMode | No field in config | `WIGGUM_EDITOR_MODE=normal` |
| 4.4 | Claude no config file | `~/.claude.json` doesn't exist | `WIGGUM_EDITOR_MODE=normal` |
| 4.5 | Non-claude agent | `WIGGUM_AGENT_CMD=amp` | Skips Claude config check |
| 4.6 | Explicit override | `WIGGUM_EDITOR_MODE=vim` set before load_config | Uses explicit value |
...
- Config loaded from main project's `.wiggum/config.sh`
- Pane registry at main project's `.wiggum/panes.json`
...
| 5.4 | Session from worktree | In worktree | Session = `wiggum-<proj-basename>` |
| 5.5 | Config from worktree | In worktree | Loads `/proj/.wiggum/config.sh` |
| 5.6 | Panes.json from worktree | `wiggum list` in worktree | Reads `/proj/.wiggum/panes.json` |
| 5.7 | Spawn from worktree | `wiggum spawn worker tk-xxx` | Creates worktree under `/proj/worktrees/` |
| 5.8 | Tickets per-worktree | Worktree `.wiggum/tickets/` | Each worktree has own tickets clone |
| 5.9 | Hooks from main | Hook triggered in worktree | Uses `/proj/.wiggum/hooks/` |
| 5.10 | Prompts from main | Spawn in worktree | Uses `/proj/.wiggum/prompts/` |
...
|----|-------------|----------|
| E.1 | No tmux session | `wiggum list` → "Session not found" |
